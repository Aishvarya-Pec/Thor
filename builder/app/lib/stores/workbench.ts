...
export class WorkbenchStore {
  ...
  async ensureFileExists(filePath: string, content: string = '') {
    try {
  ...
  async createProjectFiles(files: { [filePath: string]: string }) {
    console.log('Starting project file creation...');
    console.log('Files to create:', Object.keys(files));
    const results = [];

    for (const [filePath, content] of Object.entries(files)) {
      console.log(`Creating file: ${filePath}, content length: ${content.length}`);
      const success = await this.ensureFileExists(filePath, content);
    ...
    console.log(`Project creation complete: ${successCount}/${results.length} files created successfully`);

    return results;
  }

  // Test method to verify file creation is working
  async testFileCreation() {
    console.log('Testing file creation...');
    const testFiles = {
      'test-file.txt': 'Hello, World!',
      'test-folder/test-file-2.txt': 'Test content in folder'
    };

    const results = await this.createProjectFiles(testFiles);
    return results;
  }
